...
if __name__ == "__main__":
    ...
    hsv_values_E = i.getGreenValues('left')
    hsv_values_D = i.getGreenValues('right')
    print(hsv_values_E)
    print(hsv_values_D)
    while True:
        values = i.checkGreen([hsv_values_E, hsv_values_D])
        print(values)
        print([hsv_values_E, hsv_values_D])
        if values == [False, False]:
            continue
        else:
            hub.speaker.beep(150)
